...
- `enable-ci-logs` defaults to `true` everywhere, adding tee/tar/metadata/S3/CloudWatch overhead to every run whether or not the caller needs shipped logs.
...
| File | Change |
|---|---|
| `cdk-review.yml` | Single `cdk synth` (with `CDK_NAG=true` always set — additive/informational, doesn't change synth output) reused for both the Nag grep and `cdk diff --app cdk.out` (removes the 2nd/3rd re-synth). Add job-level `concurrency: { group: cdk-review-${{ github.ref }}, cancel-in-progress: true }`. Flip `enable-ci-logs` default to `false`. Timeout → 10 min. |
| `cdk-deploy.yml` | Add explicit `secrets: { AWS_ROLE_ARN: { required: true } }`. Add a `cdk synth` step immediately before deploy; `cdk deploy --app cdk.out` reuses it. Smoke test hard-fails on bad status (new `smoke-test-required` input, default `true`). Flip `enable-ci-logs` default to `false`. Timeout → 15 min. |
| `static-site-review.yml` / `static-site-deploy.yml` | Mirror the same synth-reuse, concurrency, secrets-block, smoke-test-hard-fail, and ci-logs-default treatment as their CDK counterparts, so the two families stop drifting. Timeouts → 10 min / 20 min respectively. |
| `python-ci.yml` | Add job-level `concurrency` (replaces the comment-only convention that was never enforced). Flip `enable-ci-logs` default to `false`. Timeout → 12 min. |
...
| `gitleaks.yml`, `validate-bucket-names.yml`, `repo-backup.yml` | Timeout tightened per table above. No other changes. |
| New: `actions/log-metadata/action.yml` | Extract the duplicated 30-line "generate log metadata" Python block (currently inlined in 4 workflows) into a composite action; each workflow calls it instead of inlining. |
...
2. Pilot caller PR — `frr-cdk` (already has all four workflow types: backup, cdk-review, cdk-deploy, security) — validate end-to-end before fanning out.
3. Remaining ~19 caller PRs, opened in batches by change type: (a) `security.yml` migrations, (b) trigger/path-filter/ci-logs cleanup on the rest.
